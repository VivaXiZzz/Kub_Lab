...
int ms(int *x, int *n) {
    if (x==n)return 0; 
    if(*x%2==0)return (*x) + ms(x+1, n);
    return (*x) + ms(x + 1, n);
}

int sc(int* x,int n) {
    if (n == 0)return sc(x,n-1);
    if (n % 2 == 0)return x[n] + sc(x, n - 1);
    return sc(x,n-1);
}

void udc(int* x, int n,int sum) {
    if (n == 0)return udc(x, n-1,sum);
    if (x[n] > 9 && x[n] < 100) { x[n] *= x[n] - sum;return udc(x, n - 1, sum); }
